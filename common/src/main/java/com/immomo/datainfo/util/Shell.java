...
abstract public class Shell {
    ...
    /** Centralized logic to discover and validate the sanity of the Avalon
     *  home directory. Returns either NULL or a directory that exists and
     *  was specified via either -Davalon.home.dir or the AVALON_HOME ENV
     *  variable.  This does a lot of work so it should only be called
     *  privately for initialization once per process.
     **/
    private static String checkAvalonHome() {

        // first check the Dflag hadoop.home.dir with JVM scope
        String home = System.getProperty("avalon.home.dir");

        // fall back to the system/user-global env variable
        if (home == null) {
            home = System.getenv("AVALON_HOME");
        }

        try {
            // couldn't find either setting for hadoop's home directory
            if (home == null) {
                throw new IOException("AVALON_HOME or avalon.home.dir are not set.");
            }
            ...
            if (!homedir.isAbsolute() || !homedir.exists() || !homedir.isDirectory()) {
                throw new IOException("Avalon home directory " + homedir
    ...
    private static String AVALON_HOME_DIR = checkAvalonHome();

    // Public getter, throws an exception if AVALON_HOME failed validation
    // checks and is being referenced downstream.
    public static final String getHadoopHome() throws IOException {
        if (AVALON_HOME_DIR == null) {
            throw new IOException("Misconfigured AVALON_HOME cannot be referenced.");
        }

        return AVALON_HOME_DIR;
    }

    /** fully qualify the path to a binary that should be in a known avalon
     *  bin location. This is primarily useful for disambiguating call-outs
     *  to executable sub-components of avalon to avoid clashes with other
    ...
    public static final String getQualifiedBinPath(String executable)
        ...
        String fullExeName = AVALON_HOME_DIR + File.separator + "bin"
                + File.separator + executable;

        File exeFile = new File(fullExeName);
        if (!exeFile.exists()) {
            throw new IOException("Could not locate executable " + fullExeName
                    + " in the Avalon binaries.");
        }
