...
import static com.immomo.datainfo.rpc.RpcConstants.AUTHORIZATION_FAILED_CALL_ID;
import static com.immomo.datainfo.rpc.RpcConstants.CONNECTION_CONTEXT_CALL_ID;
import static com.immomo.datainfo.rpc.RpcConstants.CURRENT_VERSION;
import static com.immomo.datainfo.rpc.RpcConstants.PING_CALL_ID;
...
import com.immomo.datainfo.io.DataOutputBuffer;
import com.immomo.datainfo.io.RpcWrapper;
import com.immomo.datainfo.io.WritableUtils;
import com.immomo.datainfo.rpc.RPC.RpcInvoker;
import com.immomo.datainfo.rpc.RPC.VersionMismatch;
import com.immomo.datainfo.net.NetUtils;
import com.immomo.datainfo.util.StringUtils;
...
import com.immomo.datainfo.conf.Configuration.IntegerRanges;
...
import com.immomo.datainfo.rpc.protobuf.RpcHeaderProtos.RpcResponseHeaderProto.RpcErrorCodeProto;
import com.immomo.datainfo.rpc.protobuf.RpcHeaderProtos.RpcResponseHeaderProto.RpcStatusProto;

import com.immomo.datainfo.rpc.protobuf.IpcConnectionContextProtos.IpcConnectionContextProto;
import com.immomo.datainfo.rpc.protobuf.RpcHeaderProtos.RpcKindProto;


/** An abstract IPC service.  IPC calls take a single {@link RpcWrapper} as a
 * parameter, and return a {@link RpcWrapper} as their value.  A service runs on
...
public abstract class Server {
    ...
    static final String RECEIVED_HTTP_REQ_RESPONSE =
            "HTTP/1.1 404 Not Found\r\n" +
                    ...
                    "It looks like you are making an HTTP request to a Avalon RPC port. " +
    ...
    static class RpcKindMapValue {
        final Class<? extends RpcWrapper> rpcRequestWrapperClass;
        final RpcInvoker rpcInvoker;
        RpcKindMapValue (Class<? extends RpcWrapper> rpcRequestWrapperClass,
    ...
    public static void registerProtocolEngine(RPC.RpcKind rpcKind,
                                              Class<? extends RpcWrapper> rpcRequestWrapperClass,
                                              RpcInvoker rpcInvoker) {
        RpcKindMapValue  old =
    ...
    public Class<? extends RpcWrapper> getRpcRequestWrapper(
     ...
     * {@link #call(RpcWrapper, long)} implementations, and under {@link RpcWrapper}
    ...
    private Class<? extends RpcWrapper> rpcRequestClass;   // class used for deserializing the rpc request
    ...
    static Class<? extends BlockingQueue<Call>> getQueueClass(
        ...
        String name = prefix + "." + CommonConfigurationKeys.RPC_CALLQUEUE_IMPL_KEY;
    ...
    private String getQueueClassPrefix() {
        return CommonConfigurationKeys.RPC_CALLQUEUE_NAMESPACE + "." + port;
    }
    ...
    public static class Call implements Schedulable {
        ...
        private final RpcWrapper rpcRequest;    // Serialized Rpc request from client
        ...
        public Call(int id, int retryCount, RpcWrapper param,
                    Connection connection) {
            this(id, retryCount, param, connection, RPC.RpcKind.RPC_BUILTIN,
                    RpcConstants.DUMMY_CLIENT_ID);
        }

        public Call(int id, int retryCount, RpcWrapper param, Connection connection,
    ...
    private class Listener extends Thread {
        ...
        private int backlogLength = conf.getInt(
                CommonConfigurationKeys.RPC_SERVER_LISTEN_QUEUE_SIZE_KEY,
                CommonConfigurationKeys.RPC_SERVER_LISTEN_QUEUE_SIZE_DEFAULT);
        ...
        public Listener() throws IOException {
            ...
            this.setName("RPC Server listener on " + port);
            this.setDaemon(true);
        }
    ...
    public class Connection {
        ...
        private final Call fakeFailedCall = new Call(AUTHORIZATION_FAILED_CALL_ID,
                RpcConstants.INVALID_RETRY_COUNT, null, this);
        private ByteArrayOutputStream fakeFailedResponse = new ByteArrayOutputStream();
        ...
        private void doFakeReply(Exception ioe) throws IOException {
            setupResponse(fakeFailedResponse, fakeFailedCall,
                    RpcStatusProto.FATAL, RpcErrorCodeProto.FATAL_UNAUTHORIZED,
                    null, ioe.getClass().getName(), ioe.getLocalizedMessage());
            responder.doRespond(fakeFailedCall);
        }
        ...
        private AuthProtocol initializeAuthContext(int authType)
            ...
            if (authProtocol == null) {
                IOException ioe = new RpcException("Unknown auth protocol:" + authType);
                doFakeReply(ioe);
                throw ioe;
            }
        ...
        private void setupBadVersionResponse(int clientVersion) throws IOException {
            String errMsg = "Server RPC version " + CURRENT_VERSION +
        ...
        private void processOneRpc(byte[] buf)
            ...
            try {
                ...
                setupResponse(fakeFailedResponse, call,
        ...
        private void processRpcRequest(RpcRequestHeaderProto header,
            ...
            Class<? extends RpcWrapper> rpcRequestClass =
    ...
    private class Handler extends Thread {
        public Handler(int instanceNumber) {
            this.setDaemon(true);
            this.setName("RPC Server handler "+ instanceNumber + " on " + port);
        }
        ...
        public void run() {
            ...
            while (running) {
                try {
                    ...
                    LOG.info(Thread.currentThread().getName() + ": " + call + " for RpcKind " + call.rpcKind);
                    if (LOG.isDebugEnabled()) {
                    ...
                    synchronized (call.connection.responseQueue) {
                        ...
                        LOG.info("returnStatus:" + returnStatus.toString() + ",detailError:" + detailedErr);
                    ...
                    LOG.info("Handler call finished.");
                }
            }
    ...
    protected Server(String bindAddress, int port,
                     Class<? extends RpcWrapper> paramClass, int handlerCount,
                     Configuration conf)
            throws IOException
    {
    ...
    protected Server(String bindAddress, int port,
                     Class<? extends RpcWrapper> rpcRequestClass, int handlerCount,
    ...
    protected Server(String bindAddress, int port,
        ...
        this.maxDataLength = conf.getInt(CommonConfigurationKeys.RPC_MAXIMUM_DATA_LENGTH,
                CommonConfigurationKeys.RPC_MAXIMUM_DATA_LENGTH_DEFAULT);
        ...
        this.maxRespSize = conf.getInt(
                CommonConfigurationKeys.RPC_SERVER_RPC_MAX_RESPONSE_SIZE_KEY,
                CommonConfigurationKeys.RPC_SERVER_RPC_MAX_RESPONSE_SIZE_DEFAULT);
        if (numReaders != -1) {
        ...
        this.readerPendingConnectionQueue = conf.getInt(
                CommonConfigurationKeys.RPC_SERVER_RPC_READ_CONNECTION_QUEUE_SIZE_KEY,
                CommonConfigurationKeys.RPC_SERVER_RPC_READ_CONNECTION_QUEUE_SIZE_DEFAULT);
        LOG.info("Server:" + maxDataLength + "," + maxQueueSize + "," + maxRespSize + "," + readThreads + "," + readerPendingConnectionQueue);
        ...
        this.tcpNoDelay = conf.getBoolean(
                CommonConfigurationKeys.RPC_SERVER_TCPNODELAY_KEY,
                CommonConfigurationKeys.RPC_SERVER_TCPNODELAY_DEFAULT);
    ...
    private void setupResponse(ByteArrayOutputStream responseBuf,
                               ...
                               RpcWrapper rv, String errorClass, String error)
            throws IOException {
        responseBuf.reset();
     ...
     *  RpcWrapper, long)} instead
     */
    @Deprecated
    public RpcWrapper call(RpcWrapper param, long receiveTime) throws Exception {
        return call(RPC.RpcKind.RPC_BUILTIN, null, param, receiveTime);
    }

    /** Called for each call. */
    public abstract RpcWrapper call(RPC.RpcKind rpcKind, String protocol,
                                  RpcWrapper param, long receiveTime) throws Exception;
    ...
    private class ConnectionManager {
        ...
        ConnectionManager() {
            this.idleScanTimer = new Timer(
                    "RPC Server idle connection scanner for port " + getPort(), true);
            this.idleScanThreshold = conf.getInt(
                    CommonConfigurationKeys.RPC_CLIENT_IDLETHRESHOLD_KEY,
                    CommonConfigurationKeys.RPC_CLIENT_IDLETHRESHOLD_DEFAULT);
            this.idleScanInterval = conf.getInt(
                    CommonConfigurationKeys.RPC_CLIENT_CONNECTION_IDLESCANINTERVAL_KEY,
                    CommonConfigurationKeys.RPC_CLIENT_CONNECTION_IDLESCANINTERVAL_DEFAULT);
            this.maxIdleTime = 2 * conf.getInt(
                    CommonConfigurationKeys.RPC_CLIENT_CONNECTION_MAXIDLETIME_KEY,
                    CommonConfigurationKeys.RPC_CLIENT_CONNECTION_MAXIDLETIME_DEFAULT);
            this.maxIdleToClose = conf.getInt(
                    CommonConfigurationKeys.RPC_CLIENT_KILL_MAX_KEY,
                    CommonConfigurationKeys.RPC_CLIENT_KILL_MAX_DEFAULT);
